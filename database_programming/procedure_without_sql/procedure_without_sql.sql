...
DROP DATABASE IF EXISTS procedimiento_sin_sql;
CREATE DATABASE procedimiento_sin_sql;
USE procedimiento_sin_sql;
...
CREATE PROCEDURE real_number(IN numero INT, OUT resultado VARCHAR(20))
BEGIN
	IF numero < 0 THEN 
...
CALL saludo() AS @saludar;
CALL real_number(3, @res);
CALL calificacion_if(7.5, @nota_res_if);
CALL calificacion_case(4.8, @nota_res);
CALL dia_semana_if(3, @dia_nombre);
CALL dia_semana_case(6, @dia_nombre_case);

SELECT 
@saludar AS saludo,
@res AS resultado_real,
@nota_res_if AS resultado_nota_if,
@nota_res AS resultado_nota_case,
@dia_nombre AS dia_if,
@dia_nombre_case AS dia_case;
